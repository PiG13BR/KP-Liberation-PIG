if (KPLIB_civinfo_debug > 0) then {[format ["Spawned on: %1", KPLIB_debugSource], "CIVINFO"] remoteExecCall ["KPLIB_fnc_log", 2];};

private _spawn_marker = [2000,999999,false] call KPLIB_fnc_getOpforSpawnPoint;
private _roadObj = [markerPos _spawn_marker, 400, []] call BIS_fnc_nearestRoad;

if (isNull _roadObj) exitWith {if (KPLIB_civinfo_debug > 0) then {["No road found", "CIVINFO"] remoteExecCall ["KPLIB_fnc_log", 2];};};

private _veh = createVehicle [KPLIB_o_mrap, getPos _roadObj, [], 0, "NONE"];
_veh setDir (getDir _roadObj);
{
    if ((_x find "FWheel") != -1) then {
        _veh setHitPointDamage [_x, 1];
    };
} forEach ((getAllHitPointsDamage _veh) select 0);

private _grp = createGroup [KPLIB_side_enemy, true];
private _hvt = [KPLIB_o_officer, getPos _roadObj, _grp, "CAPTAIN", 30] call KPLIB_fnc_createManagedUnit;

for "_i" from 1 to 4 do {
    [selectRandom KPLIB_o_militiaInfantry, _grp, _grp] call KPLIB_fnc_createManagedUnit;
    sleep 0.1;
};

private _waypoint = _grp addWaypoint [getPos _roadObj, 100];
_waypoint setWaypointType "MOVE";
_waypoint setWaypointBehaviour "SAFE";
_waypoint setWaypointCombatMode "YELLOW";
_waypoint setWaypointSpeed "LIMITED";
_waypoint setWaypointCompletionRadius 10;

_waypoint = _grp addWaypoint [getPos _roadObj, 100];
_waypoint setWaypointType "MOVE";
_waypoint = _grp addWaypoint [getPos _roadObj, 100];
_waypoint setWaypointType "MOVE";
_waypoint = _grp addWaypoint [getPos _roadObj, 100];
_waypoint setWaypointType "MOVE";

_waypoint = _grp addWaypoint [getPos _roadObj, 100];
_waypoint setWaypointType "CYCLE";

if (KPLIB_civinfo_debug > 0) then {[format ["Vehicle and group created on: %1", KPLIB_debugSource], "CIVINFO"] remoteExecCall ["KPLIB_fnc_log", 2];};

private _marker_pos = [((((getPos _roadObj) select 0) + 200) - random 400),((((getPos _roadObj) select 1) + 200) - random 400),0];

[4, _marker_pos] remoteExec ["civinfo_notifications"];

private _time_remaining = KPLIB_civinfo_task_duration;

while {(alive _hvt) && _time_remaining > 0} do {
    uiSleep 1;
    private _player_near = false;
    {
        if (((_x distance _veh) < 1000) && (alive _x)) exitWith {_player_near = true};
    } foreach allPlayers;

    if !(_player_near) then {
        _time_remaining = _time_remaining - 1;
    };
    if ((KPLIB_civinfo_debug > 0) && ((_time_remaining % 60) == 0)) then {[format ["Task will despawn in %1 minutes", round (_time_remaining / 60)], "CIVINFO"] remoteExecCall ["KPLIB_fnc_log", 2];};
};

if (KPLIB_civinfo_debug > 0) then {[format ["Loop exited on: %1", KPLIB_debugSource], "CIVINFO"] remoteExecCall ["KPLIB_fnc_log", 2];};

if (alive _hvt) then {
    deleteVehicle _veh;
    {
        if (isNull objectParent _x) then {deleteVehicle _x} else {(objectParent _x) deleteVehicleCrew _x};
    } forEach (units _grp);
    [6] remoteExec ["civinfo_notifications"];
    if (KPLIB_civinfo_debug > 0) then {["Task despawned", "CIVINFO"] remoteExecCall ["KPLIB_fnc_log", 2];};
} else {
    KPLIB_enemyReadiness = round (KPLIB_enemyReadiness * 0.6);
    if (!isServer) then {
        publicVariableServer "KPLIB_enemyReadiness";
    };
    [5] remoteExec ["civinfo_notifications"];
    if (KPLIB_civinfo_debug > 0) then {["Task ended with success", "CIVINFO"] remoteExecCall ["KPLIB_fnc_log", 2];};
};
